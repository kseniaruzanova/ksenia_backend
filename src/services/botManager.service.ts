...
class BotManager extends EventEmitter {
  ...
  private async handleStepStates(
    ...
    if (userState === "step_1") {
      ...
      const message: string | null = await this.generateAIResponseWithHistory(
        "Проанализируй контекст диалоги и на базе последнего ответа пользователя, ты должен определить контекст ответа пользователя, ключ, позицию, положительный ответ или отрицательный /n к отрицательному относятся ответы: с сомнениями, страхами, нежеланием и тд к положительному относятся: да, согласен, хочу и тд строго возвращай в ответе false или true /n true при положительном контексте /n false при негативном или отрицательном",
        messageHistory,
        "gpt-4o-mini",
        0.7,
        'openai'
