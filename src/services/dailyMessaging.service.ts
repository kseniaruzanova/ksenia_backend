    ...
    async generatePersonalizedMessage(userMessages: string[], customerId: string, chatId: string): Promise<string> {
        ...
        try {
            ...
            const prompt = {
              ...
              Анализируй последние сообщения пользователя и выбери случайную тему из переписки. 
              Сформулируй только один вопрос в свободном разговорном стиле от лица девушки. 
              В вопросе обязательно укажи, к какому сообщению или теме ты отсылаешься (например: "Ты писал про ...", "А что насчёт того, что ты говорил ..."). 
              Вопрос должен быть естественным и побуждать к ответу. 
              Не упоминай карму, мистику, эзотерику или что-то подобное. 
              ВАЖНО: НЕ повторяй вопросы, которые уже задавались ранее (список ниже). 
              Выводи только сам вопрос — без вступлений, пояснений или лишнего текста.`
                  },
                  {
                    role: "user",
