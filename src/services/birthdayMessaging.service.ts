...
class BirthdayMessagingService extends EventEmitter {
    ...
    async generateBirthdayMessage(user: BirthdayUser): Promise<string> {
        try {
            ...
            ИНСТРУКЦИЯ:
            Анализируй каждый аркан глубоко, учитывая его традиционные значения и специфику системы Кроули. Давай сбалансированную характеристику, показывая как позитивные, так и challenging аспекты.
    ...
    async sendBirthdayMessage(user: BirthdayUser): Promise<{ success: boolean; error?: string }> {
        try {
            ...
            const result = await botManager.sendMessage(user.customerId, user.chatId, birthdayMessage);
